...
describe("<SearchBox>", ()=> {
  ...
  test("Renders the search box", () => {
    const {queryByPlaceholderText, container} = render(<SearchBox {...defaultProps} />);
  ...
  test("Shows 1 matching item", ()=> {
    const {queryByPlaceholderText, queryAllByTitle, container, debug} = render(<SearchBox {...defaultProps} />);
    const inputElement = queryByPlaceholderText(/search/i);
  ...
  test("Don't use sessionStorage if useCaching is false", () => {
    const {queryByPlaceholderText} = render(<SearchBox {...defaultProps} useCaching={false} />);
    const inputElement = queryByPlaceholderText(/search/i);
  ...
  test("Use sessionStorage if useCaching is true", () => {
    const {queryByPlaceholderText} = render(<SearchBox {...defaultProps} />);
    const inputElement = queryByPlaceholderText(/search/i);
  ...
  test("Retrieve cached values", async () => {
    ...
    const {queryByPlaceholderText} = render(<SearchBox {...defaultProps} onSearch={onSearch} />);
    const inputElement = queryByPlaceholderText(/search/i);
    fireEvent.change(inputElement, { target: { value: "cachingEnabled" }});
    await delay(300);
  ...
  test("Check if onSelect is called on item selection", () => {
    ...
    const {queryByPlaceholderText, queryAllByTitle} = render(<SearchBox {...defaultProps} onSelect={onSelect} />);
    const inputElement = queryByPlaceholderText(/search/i);
    fireEvent.change(inputElement, { target: { value: "v" }});
  ...
  test("Check if onFocus is called on input focus", () => {
    ...
    const {queryByPlaceholderText} = render(<SearchBox {...defaultProps} onFocus={onFocus} />);
    const inputElement = queryByPlaceholderText(/search/i);
  ...
  test("Check if input is focused if autoFocus is true",  () => {
    const {queryByPlaceholderText} = render(<SearchBox {...defaultProps} autoFocus={true} />);
    const inputElement = queryByPlaceholderText(/search/i);
